...
- [ ] Fix the strict TypeScript error in `packages/session-clock` so the root
  `pnpm typecheck` command passes.
- [ ] Replace remaining WXT starter package metadata and README content with
  O11y Replay-specific names and instructions.
- [ ] Confirm `pnpm install`, `pnpm test`, `pnpm typecheck`, and `pnpm build`
